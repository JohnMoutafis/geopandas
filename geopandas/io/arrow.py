...
def _create_metadata(df, schema_version=None):
    ...
    for col in df.columns[df.dtypes == "geometry"]:
        ...
        # Instead of going through the `_vectorized.geometry_type_values` that do not include the "Z" varieties.
        geometry_types = sorted(Series(
            [f"{geom} Z" if has_z else geom for geom, has_z in zip(series.geom_type.unique(), series.has_z)]
        ).dropna())
